 ...
 * EVS Intel Bot - 카카오톁 챗봇 웹훅 서버
 *
 * 흐름: 카톁 메시지 수신 → URL 추출 → 크롤링 → Gemini AI 분석 → Teams 포스팅 → 카톡 응답
 */
...
// ===== 헬스체크 =====
app.get('/', (req, res) => {
  res.json({ status: 'ok', bot: 'EVS Intel Bot', version: '2.0.0' });
});
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    // 카카오 오픈빌더는 5초 타임아웃이므로, 빠른 응답 필요
    const timeoutPromise = new Promise((resolve) => {
      setTimeout(() => resolve(null), 4000); // 4초 타임아웃
    });
    ...
    if (result) {
      return res.json(buildTextResponse(result));
    } else {
      // 타임아웃 — 백그라운드에서 계속 처리
      processPromise.then(r => console.log('백그라운드 처리 완료:', r)).catch(console.error);
      return res.json(buildTextResponse(
        `📥 �{크 접수 완료!\n${url}\n\n분석 중이에요. 잠시 후 Teams 채널에서 확인하세요.`
      ));
    }
    ...
    console.error('웹훅 처리 오류:', error);
    return res.json(buildTextResponse('처리 중 오류가 발생했어요. 다시 시도해주세요.'));
  }
});
